...
template<typename T>
MyArray<T>::MyArray(MyArray&& other) noexcept :size(other.size), data(other.data) {
	other.data = nullptr;
	other.size = 0;
}
...
T& MyArray<T>::operator[](int index) {
	if (isEmpty()) {
		std::cout << "Error.Array is empty.The element is assigned the default value:";
		static T defaultValue = T();
		return defaultValue;
	}
	if (!(isValidIndex(index))) {
		static T defaultValue = T();
		std::cout << "Error:going beyond the array boundary.The element is assigned the default value:";
		return defaultValue;
	}
	else {
		return data[index];
