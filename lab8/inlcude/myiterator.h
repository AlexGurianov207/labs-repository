...
private:
	...
	int totalCols;
public:
	MyIterator(T** matrix, int rows, int cols, int startRow = 0, int startCol = 0);
...
template<typename T>
MyIterator<T>::MyIterator(const MyIterator& other):data(other.data),currentRow(other.currentRow),currentCol(other.currentCol),totalRows(other.totalRows),totalCols(other.totalCols) {
